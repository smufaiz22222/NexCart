import { faker } from '@faker-js/faker';

const CATEGORY_IMAGES = {
  Grocery: [
    'https://images.unsplash.com/photo-1542838132-92c53300491e?w=500&q=80',
    'https://images.unsplash.com/photo-1578916171728-46686eac8d58?w=500&q=80',
    'https://images.unsplash.com/photo-1506084868230-bb9d95c24759?w=500&q=80',
    'https://images.unsplash.com/photo-1488459718432-36a85e089406?w=500&q=80',
    'https://images.unsplash.com/photo-1543083501-594121ca1db5?w=500&q=80',
    'https://images.unsplash.com/photo-1610397613050-5999d1487f47?w=500&q=80',
    'https://images.unsplash.com/photo-1516594798947-e65505dbb29d?w=500&q=80',
    'https://images.unsplash.com/photo-1506806732259-39c2d0268443?w=500&q=80',
  ],
  Sports: [
    'https://images.unsplash.com/photo-1517838277536-f5f99be501cd?w=500&q=80',
    'https://images.unsplash.com/photo-1506126613408-eca07ce68773?w=500&q=80',
    'https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5?w=500&q=80',
    'https://images.unsplash.com/photo-1517649763962-0c623066013b?w=500&q=80',
    'https://images.unsplash.com/photo-1530541930197-ff16ac917b0e?w=500&q=80',
    'https://images.unsplash.com/photo-1476480862126-209bfaa8edc8?w=500&q=80',
    'https://images.unsplash.com/photo-1500485035595-cbe6f645feb1?w=500&q=80',
    'https://images.unsplash.com/photo-1518611012118-696072aa579a?w=500&q=80',
  ],
  Books: [
    'https://images.unsplash.com/photo-1497633762265-9d179a990aa6?w=500&q=80',
    'https://images.unsplash.com/photo-1544947950-fa07a98d237f?w=500&q=80',
    'https://images.unsplash.com/photo-1512820790803-83ca734da794?w=500&q=80',
    'https://images.unsplash.com/photo-1516979187457-637abb4f9353?w=500&q=80',
    'https://images.unsplash.com/photo-1532012197267-da84d127e765?w=500&q=80',
    'https://images.unsplash.com/photo-1506880018603-83d5b814b5a6?w=500&q=80',
    'https://images.unsplash.com/photo-1513001900722-370f803f498d?w=500&q=80',
    'https://images.unsplash.com/photo-1524995997946-a1c2e315a42f?w=500&q=80',
  ],
  Beauty: [
    'https://images.unsplash.com/photo-1596462502278-27bfdc403348?w=500&q=80',
    'https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?w=500&q=80',
    'https://images.unsplash.com/photo-1512496015851-a90fb38ba796?w=500&q=80',
    'https://images.unsplash.com/photo-1608248597481-496100c80836?w=500&q=80',
    'https://images.unsplash.com/photo-1556228720-195a672e8a03?w=500&q=80',
    'https://images.unsplash.com/photo-1571781926291-c477ebfd024b?w=500&q=80',
    'https://images.unsplash.com/photo-1598440947619-2c35fc9aa908?w=500&q=80',
    'https://images.unsplash.com/photo-1515688594390-b649af70d282?w=500&q=80',
  ],
  Electronics: [
    'https://images.unsplash.com/photo-1546868871-7041f2a55e12?w=500&q=80',
    'https://images.unsplash.com/photo-1526738549149-8e07eca6c147?w=500&q=80',
    'https://images.unsplash.com/photo-1588508065123-287b28e013da?w=500&q=80',
    'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=500&q=80',
    'https://images.unsplash.com/photo-1608156639585-b3a032ef9689?w=500&q=80',
    'https://images.unsplash.com/photo-1527864550417-7fd91fc51a46?w=500&q=80',
    'https://images.unsplash.com/photo-1593508512255-86ab42a8e620?w=500&q=80',
    'https://images.unsplash.com/photo-1583394838336-acd977736f90?w=500&q=80',
  ],
  Apparel: [
    'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=500&q=80',
    'https://images.unsplash.com/photo-1539109136881-3be0616acf4b?w=500&q=80',
    'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=500&q=80',
    'https://images.unsplash.com/photo-1434389677669-e08b4cac3105?w=500&q=80',
    'https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=500&q=80',
    'https://images.unsplash.com/photo-1549298916-b41d501d3772?w=500&q=80',
    'https://images.unsplash.com/photo-1606107557195-0e29a4b5b4aa?w=500&q=80',
    'https://images.unsplash.com/photo-1582562124811-c09040d0a901?w=500&q=80',
  ],
  'Home & Kitchen': [
    'https://images.unsplash.com/photo-1556911220-e15b29be8c8f?w=500&q=80',
    'https://images.unsplash.com/photo-1588854337236-6889d631faa8?w=500&q=80',
    'https://images.unsplash.com/photo-1517760444937-f6397edcbbcd?w=500&q=80',
    'https://images.unsplash.com/photo-1507089947368-19c1da9775ae?w=500&q=80',
    'https://images.unsplash.com/photo-1584622650111-993a426fbf0a?w=500&q=80',
  ],
  'Toys & Games': [
    'https://images.unsplash.com/photo-1558060370-d644479cb6f7?w=500&q=80',
    'https://images.unsplash.com/photo-1539627831859-a911cf04d3cd?w=500&q=80',
    'https://images.unsplash.com/photo-1596461404969-9ae70f2830c1?w=500&q=80',
    'https://images.unsplash.com/photo-1515488042361-404e9250afef?w=500&q=80',
    'https://images.unsplash.com/photo-1587654780291-39c9404d746b?w=500&q=80',
  ],
  Automotive: [
    'https://images.unsplash.com/photo-1486006920555-c77dce18193b?w=500&q=80',
    'https://images.unsplash.com/photo-1617469767053-d3b523a0b982?w=500&q=80',
    'https://images.unsplash.com/photo-1507136566006-cfc505b114fc?w=500&q=80',
    'https://images.unsplash.com/photo-1619642751034-765dfdf7c58e?w=500&q=80',
    'https://images.unsplash.com/photo-1552519507-da3b142c6e3d?w=500&q=80',
  ],
  'Office Supplies': [
    'https://images.unsplash.com/photo-1513151233558-d860c5398176?w=500&q=80',
    'https://images.unsplash.com/photo-1586075010923-2dd45e9b2d4f?w=500&q=80',
    'https://images.unsplash.com/photo-1457369804613-52c61a468e7d?w=500&q=80',
    'https://images.unsplash.com/photo-1504868584819-f8e8b4b6d7e3?w=500&q=80',
    'https://images.unsplash.com/photo-1497032628192-86f99bcd76bc?w=500&q=80',
  ],
  'Pet Supplies': [
    'https://images.unsplash.com/photo-1516734212186-a967f81ad0d7?w=500&q=80',
    'https://images.unsplash.com/photo-1583511655857-d19b40a7a54e?w=500&q=80',
    'https://images.unsplash.com/photo-1541599540903-216a46cf1dd0?w=500&q=80',
    'https://images.unsplash.com/photo-1535268647977-a403b69fc756?w=500&q=80',
    'https://images.unsplash.com/photo-1544568100-847a948585b9?w=500&q=80',
  ],
};

function getRandomImage(category) {
  const images = CATEGORY_IMAGES[category] || CATEGORY_IMAGES.Grocery;
  return images[Math.floor(Math.random() * images.length)];
}

export default async function seedProducts(prisma, wholesalers) {
  console.log('📦 Seeding Products (Scaling to 2000+ items)...');

  const w1 = wholesalers.find((w) => w.businessName === 'ElectroHub Pro').id;
  const w2 = wholesalers.find((w) => w.businessName === 'Urban Threads').id;
  const wGrocery = wholesalers.find((w) => w.businessName === 'Daily Groceries Co.').id;
  const wSports = wholesalers.find((w) => w.businessName === 'Active Sports Gear').id;
  const wBooks = wholesalers.find((w) => w.businessName === 'Bookworm Press').id;
  const wBeauty = wholesalers.find((w) => w.businessName === 'Glow Beauty').id;
  const wHome = wholesalers.find((w) => w.businessName === 'Homeware Depot').id;
  const wToys = wholesalers.find((w) => w.businessName === 'Toyland Wholesalers').id;
  const wAuto = wholesalers.find((w) => w.businessName === 'Apex Auto Parts').id;
  const wOffice = wholesalers.find((w) => w.businessName === 'Office Express').id;
  const wPets = wholesalers.find((w) => w.businessName === 'Pet Palace Wholesale').id;

  // 1. Core Hardcoded Products (100)
  console.log('  - Mapping 100 original hardcoded products...');
  const hardcodedProducts = [
    // --- TECH PRODUCTS (w1) ---
    {
      wholesalerId: w1,
      name: 'Pro Smartphone 128GB',
      description: 'Latest gen smartphone',
      price: 899,
      costPrice: 600,
      sku: 'ELEC-001',
      currentStock: 45,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Pro Smartphone 256GB',
      description: 'More storage',
      price: 999,
      costPrice: 650,
      sku: 'ELEC-002',
      currentStock: 30,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1592890288564-76628a30a657?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Gaming Laptop 15-inch',
      description: 'High performance',
      price: 1299,
      costPrice: 900,
      sku: 'ELEC-003',
      currentStock: 20,
      minStock: 5,
      imageUrl: 'https://images.unsplash.com/photo-1603302576837-37561b2e2302?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Office Laptop 14-inch',
      description: 'Light and fast',
      price: 799,
      costPrice: 500,
      sku: 'ELEC-004',
      currentStock: 50,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1496181133206-80ce9b88a853?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Wireless Earbuds',
      description: 'Noise cancelling',
      price: 149,
      costPrice: 80,
      sku: 'ELEC-005',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Over-Ear Headphones',
      description: 'Studio quality',
      price: 299,
      costPrice: 150,
      sku: 'ELEC-006',
      currentStock: 60,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Smartwatch Series 5',
      description: 'Health tracking',
      price: 249,
      costPrice: 130,
      sku: 'ELEC-007',
      currentStock: 40,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Fitness Tracker',
      description: 'Step counter',
      price: 99,
      costPrice: 40,
      sku: 'ELEC-008',
      currentStock: 150,
      minStock: 30,
      imageUrl: 'https://images.unsplash.com/photo-1575311373937-040b8e1fd5b6?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: '4K LED TV 55-inch',
      description: 'Smart TV',
      price: 599,
      costPrice: 350,
      sku: 'ELEC-009',
      currentStock: 15,
      minStock: 5,
      imageUrl: 'https://images.unsplash.com/photo-1593359677879-a4bb92f829d1?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'OLED TV 65-inch',
      description: 'Premium display',
      price: 1499,
      costPrice: 900,
      sku: 'ELEC-010',
      currentStock: 10,
      minStock: 2,
      imageUrl: 'https://images.unsplash.com/photo-1593784991095-a205029471b6?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Bluetooth Speaker',
      description: 'Waterproof portable',
      price: 59,
      costPrice: 25,
      sku: 'ELEC-011',
      currentStock: 80,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1608156639585-b3a032ef9689?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Home Theater System',
      description: '5.1 Surround',
      price: 399,
      costPrice: 200,
      sku: 'ELEC-012',
      currentStock: 25,
      minStock: 5,
      imageUrl: 'https://images.unsplash.com/photo-1545454675-3531bdf9915e?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Mechanical Keyboard',
      description: 'RGB Switches',
      price: 109,
      costPrice: 50,
      sku: 'ELEC-013',
      currentStock: 60,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1511467687858-23d96c32e4ae?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Wireless Mouse',
      description: 'Ergonomic',
      price: 49,
      costPrice: 20,
      sku: 'ELEC-014',
      currentStock: 120,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1527864550417-7fd91fc51a46?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Gaming Mouse',
      description: 'High DPI',
      price: 69,
      costPrice: 30,
      sku: 'ELEC-015',
      currentStock: 90,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1615663245857-ac93bb7c39e7?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'USB-C Hub',
      description: '7-in-1 dongle',
      price: 39,
      costPrice: 15,
      sku: 'ELEC-016',
      currentStock: 200,
      minStock: 40,
      imageUrl: 'https://images.unsplash.com/photo-1618410320928-25228d811631?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'External SSD 1TB',
      description: 'Fast storage',
      price: 129,
      costPrice: 70,
      sku: 'ELEC-017',
      currentStock: 75,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1597872200349-0160826143a8?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'External HDD 2TB',
      description: 'Backup storage',
      price: 79,
      costPrice: 40,
      sku: 'ELEC-018',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1531062991700-2f3ef4131501?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Webcam 1080p',
      description: 'Clear video calls',
      price: 59,
      costPrice: 25,
      sku: 'ELEC-019',
      currentStock: 150,
      minStock: 30,
      imageUrl: 'https://images.unsplash.com/photo-1626125332214-7bc77f7f3299?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Webcam 4K',
      description: 'Pro streaming',
      price: 149,
      costPrice: 80,
      sku: 'ELEC-020',
      currentStock: 40,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1551817311-d70c58232147?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Wi-Fi 6 Router',
      description: 'Fast internet',
      price: 199,
      costPrice: 100,
      sku: 'ELEC-021',
      currentStock: 50,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1544197150-b99a580bb7a8?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Mesh Wi-Fi System',
      description: 'Whole home coverage',
      price: 299,
      costPrice: 150,
      sku: 'ELEC-022',
      currentStock: 30,
      minStock: 5,
      imageUrl: 'https://images.unsplash.com/photo-1625842268584-8f3bf9ff967e?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Smart Plug',
      description: 'Voice controlled',
      price: 19,
      costPrice: 8,
      sku: 'ELEC-023',
      currentStock: 300,
      minStock: 50,
      imageUrl: 'https://images.unsplash.com/photo-1558002038-1055907df827?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Smart Bulb',
      description: 'RGB LED',
      price: 24,
      costPrice: 10,
      sku: 'ELEC-024',
      currentStock: 250,
      minStock: 50,
      imageUrl: 'https://images.unsplash.com/photo-1550524514-9636edba3118?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Action Camera',
      description: '4K 60fps',
      price: 349,
      costPrice: 200,
      sku: 'ELEC-025',
      currentStock: 35,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1526170315870-ef687299c1ad?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Drone with Camera',
      description: 'Foldable design',
      price: 499,
      costPrice: 300,
      sku: 'ELEC-026',
      currentStock: 20,
      minStock: 5,
      imageUrl: 'https://images.unsplash.com/photo-1473968512647-3e44a224fe8f?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Tablet 10-inch',
      description: 'For reading and video',
      price: 299,
      costPrice: 150,
      sku: 'ELEC-027',
      currentStock: 80,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1544244015-0df4b3ffc6b0?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'E-Reader',
      description: 'Paper-like display',
      price: 129,
      costPrice: 70,
      sku: 'ELEC-028',
      currentStock: 60,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Portable Power Bank',
      description: '10000mAh',
      price: 29,
      costPrice: 12,
      sku: 'ELEC-029',
      currentStock: 200,
      minStock: 40,
      imageUrl: 'https://images.unsplash.com/photo-1609091839311-d5365f9ff1c5?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'High-Cap Power Bank',
      description: '20000mAh',
      price: 49,
      costPrice: 22,
      sku: 'ELEC-030',
      currentStock: 150,
      minStock: 30,
      imageUrl: 'https://images.unsplash.com/photo-1625772299848-391b6a87d7b3?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Fast Wall Charger',
      description: '65W GaN',
      price: 35,
      costPrice: 15,
      sku: 'ELEC-031',
      currentStock: 180,
      minStock: 40,
      imageUrl: 'https://images.unsplash.com/photo-1619145121172-e42967679803?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Wireless Charging Pad',
      description: 'Qi Certified',
      price: 25,
      costPrice: 10,
      sku: 'ELEC-032',
      currentStock: 120,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1616422285623-13ff0167c958?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'USB-C Cable 6ft',
      description: 'Braided cord',
      price: 15,
      costPrice: 5,
      sku: 'ELEC-033',
      currentStock: 500,
      minStock: 100,
      imageUrl: 'https://images.unsplash.com/photo-1619145122116-29a0083e4708?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Lightning Cable 6ft',
      description: 'MFi Certified',
      price: 18,
      costPrice: 6,
      sku: 'ELEC-034',
      currentStock: 450,
      minStock: 100,
      imageUrl: 'https://images.unsplash.com/photo-1611002214172-792c1f90b59a?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Monitor 24-inch',
      description: '1080p IPS',
      price: 149,
      costPrice: 80,
      sku: 'ELEC-035',
      currentStock: 70,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1527443224154-c4a3942d3acf?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Monitor 27-inch',
      description: '1440p 144Hz',
      price: 299,
      costPrice: 180,
      sku: 'ELEC-036',
      currentStock: 40,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1547115941-07904209cff7?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Microphone',
      description: 'USB Condenser',
      price: 99,
      costPrice: 40,
      sku: 'ELEC-037',
      currentStock: 60,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1590602847861-f357a9332bbc?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Ring Light',
      description: '10-inch LED',
      price: 39,
      costPrice: 15,
      sku: 'ELEC-038',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1589330694653-ded6df03f754?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Tripod Stand',
      description: 'Adjustable height',
      price: 29,
      costPrice: 12,
      sku: 'ELEC-039',
      currentStock: 120,
      minStock: 25,
      imageUrl: 'https://images.unsplash.com/photo-1590251144053-5d590494481d?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Graphics Card RTX',
      description: '8GB VRAM',
      price: 499,
      costPrice: 350,
      sku: 'ELEC-040',
      currentStock: 15,
      minStock: 5,
      imageUrl: 'https://images.unsplash.com/photo-1591488320449-011701bb6704?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'CPU 8-Core',
      description: 'High frequency',
      price: 299,
      costPrice: 200,
      sku: 'ELEC-041',
      currentStock: 30,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1591799264318-7e6ef8ddb7ea?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Motherboard ATX',
      description: 'WiFi included',
      price: 199,
      costPrice: 120,
      sku: 'ELEC-042',
      currentStock: 40,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1518770660439-4636190af475?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'RAM 16GB Kit',
      description: '3200MHz DDR4',
      price: 79,
      costPrice: 40,
      sku: 'ELEC-043',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1562976540-1502c2145186?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'PC Case',
      description: 'Tempered Glass',
      price: 89,
      costPrice: 45,
      sku: 'ELEC-044',
      currentStock: 25,
      minStock: 5,
      imageUrl: 'https://images.unsplash.com/photo-1587202377425-8b6033bc3ad2?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Power Supply 750W',
      description: '80+ Gold',
      price: 109,
      costPrice: 60,
      sku: 'ELEC-045',
      currentStock: 40,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1587302912306-cf1ed9c33146?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Smart Lock',
      description: 'Keypad entry',
      price: 149,
      costPrice: 80,
      sku: 'ELEC-046',
      currentStock: 50,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1558002038-1055907df827?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Video Doorbell',
      description: '1080p camera',
      price: 99,
      costPrice: 50,
      sku: 'ELEC-047',
      currentStock: 70,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1558002038-1055907df827?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Security Camera',
      description: 'Indoor/Outdoor',
      price: 79,
      costPrice: 40,
      sku: 'ELEC-048',
      currentStock: 90,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1557324232-b8917d3c3dcb?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w1,
      name: 'Smart Thermostat',
      description: 'Energy saving',
      price: 199,
      costPrice: 100,
      sku: 'ELEC-049',
      currentStock: 30,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1567393528677-d6dfb7d8b52c?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Robot Vacuum',
      description: 'Auto mapping',
      price: 249,
      costPrice: 130,
      sku: 'ELEC-050',
      currentStock: 25,
      minStock: 5,
      imageUrl: 'https://images.unsplash.com/photo-1518133835878-5a93cc3f89e5?w=500&q=80',
      category: 'Electronics',
      sizes: [],
    },

    // --- APPAREL PRODUCTS (w2) ---
    {
      wholesalerId: w2,
      name: 'Classic T-Shirt Black',
      description: '100% Cotton',
      price: 20,
      costPrice: 8,
      sku: 'APP-051',
      currentStock: 200,
      minStock: 50,
      imageUrl: 'https://images.unsplash.com/photo-1503342217505-b0a15ec3261c?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Classic T-Shirt White',
      description: '100% Cotton',
      price: 20,
      costPrice: 8,
      sku: 'APP-052',
      currentStock: 200,
      minStock: 50,
      imageUrl: 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'V-Neck T-Shirt Blue',
      description: 'Slim fit',
      price: 25,
      costPrice: 10,
      sku: 'APP-053',
      currentStock: 150,
      minStock: 30,
      imageUrl: 'https://images.unsplash.com/photo-1618354691373-d851c5c3a990?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Graphic Tee Vintage',
      description: 'Retro print',
      price: 30,
      costPrice: 12,
      sku: 'APP-054',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1562157873-818bc0726f68?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Long Sleeve Henley',
      description: 'Ribbed cotton',
      price: 35,
      costPrice: 15,
      sku: 'APP-055',
      currentStock: 80,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1617114919297-3c8ddb01f599?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Polo Shirt Navy',
      description: 'Breathable mesh',
      price: 40,
      costPrice: 18,
      sku: 'APP-056',
      currentStock: 120,
      minStock: 25,
      imageUrl: 'https://images.unsplash.com/photo-1581655353564-df123a1eb820?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Polo Shirt Red',
      description: 'Breathable mesh',
      price: 40,
      costPrice: 18,
      sku: 'APP-057',
      currentStock: 100,
      minStock: 25,
      imageUrl: 'https://images.unsplash.com/photo-1586363104862-3a5e2ab60d99?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Denim Jacket Light',
      description: 'Vintage wash',
      price: 89,
      costPrice: 40,
      sku: 'APP-058',
      currentStock: 40,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1576995853123-5a10305d93c0?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Denim Jacket Dark',
      description: 'Raw indigo',
      price: 89,
      costPrice: 40,
      sku: 'APP-059',
      currentStock: 40,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1601333144130-8cbb312386b6?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Leather Jacket Black',
      description: 'Genuine leather',
      price: 199,
      costPrice: 90,
      sku: 'APP-060',
      currentStock: 20,
      minStock: 5,
      imageUrl: 'https://images.unsplash.com/photo-1551028719-00167b16eac5?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Bomber Jacket Olive',
      description: 'Lightweight zip',
      price: 79,
      costPrice: 35,
      sku: 'APP-061',
      currentStock: 60,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Windbreaker Neon',
      description: 'Water resistant',
      price: 69,
      costPrice: 30,
      sku: 'APP-062',
      currentStock: 50,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1544441893-675973e31985?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Fleece Hoodie Grey',
      description: 'Warm and soft',
      price: 59,
      costPrice: 25,
      sku: 'APP-063',
      currentStock: 150,
      minStock: 30,
      imageUrl: 'https://images.unsplash.com/photo-1556821840-3a63f95609a7?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Pullover Hoodie Black',
      description: 'Classic fit',
      price: 55,
      costPrice: 22,
      sku: 'APP-064',
      currentStock: 180,
      minStock: 40,
      imageUrl: 'https://images.unsplash.com/photo-1578587018452-892bacefd3f2?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Crewneck Sweatshirt',
      description: 'Vintage athletic',
      price: 49,
      costPrice: 20,
      sku: 'APP-065',
      currentStock: 120,
      minStock: 25,
      imageUrl: 'https://images.unsplash.com/photo-1620799140408-edc6dcb6d633?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Slim Fit Jeans Blue',
      description: 'Stretch denim',
      price: 69,
      costPrice: 30,
      sku: 'APP-066',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1542272604-787c3835535d?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Straight Jeans Black',
      description: 'Classic cut',
      price: 69,
      costPrice: 30,
      sku: 'APP-067',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1541099649105-f69ad21f3246?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Chino Pants Khaki',
      description: 'Office ready',
      price: 59,
      costPrice: 25,
      sku: 'APP-068',
      currentStock: 120,
      minStock: 25,
      imageUrl: 'https://images.unsplash.com/photo-1473966968600-fa801b869a1a?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Chino Pants Navy',
      description: 'Office ready',
      price: 59,
      costPrice: 25,
      sku: 'APP-069',
      currentStock: 120,
      minStock: 25,
      imageUrl: 'https://images.unsplash.com/photo-1594932224828-b4b059b6f6f9?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Cargo Pants Olive',
      description: 'Multiple pockets',
      price: 65,
      costPrice: 28,
      sku: 'APP-070',
      currentStock: 80,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1517445312882-bc9910d016b7?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Joggers Grey',
      description: 'Athleisure wear',
      price: 45,
      costPrice: 18,
      sku: 'APP-071',
      currentStock: 150,
      minStock: 30,
      imageUrl: 'https://images.unsplash.com/photo-1552902865-b72c031ac5ea?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Athletic Shorts Black',
      description: 'Quick dry',
      price: 35,
      costPrice: 14,
      sku: 'APP-072',
      currentStock: 200,
      minStock: 40,
      imageUrl: 'https://images.unsplash.com/photo-1591195853828-11db59a44f6b?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Board Shorts Floral',
      description: 'Swimwear',
      price: 40,
      costPrice: 16,
      sku: 'APP-073',
      currentStock: 90,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1559132103-6252924376f9?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Casual Button-Down',
      description: 'Plaid flannel',
      price: 55,
      costPrice: 24,
      sku: 'APP-074',
      currentStock: 110,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1523381210434-271e8be1f52b?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Dress Shirt White',
      description: 'Wrinkle free',
      price: 65,
      costPrice: 28,
      sku: 'APP-075',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1598033129183-c4f50c7176c8?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Dress Shirt Blue',
      description: 'Wrinkle free',
      price: 65,
      costPrice: 28,
      sku: 'APP-076',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1594932224828-b4b059b6f6f9?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Blazer Navy',
      description: 'Tailored fit',
      price: 149,
      costPrice: 65,
      sku: 'APP-077',
      currentStock: 40,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Summer Dress Floral',
      description: 'Lightweight rayon',
      price: 59,
      costPrice: 25,
      sku: 'APP-078',
      currentStock: 80,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1572804013307-f9a8a97ee04b?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Maxi Dress Black',
      description: 'Elegant evening',
      price: 89,
      costPrice: 40,
      sku: 'APP-079',
      currentStock: 60,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1515372039744-b8f02a3ae446?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Pleated Skirt',
      description: 'Midi length',
      price: 49,
      costPrice: 20,
      sku: 'APP-080',
      currentStock: 70,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1583337130417-3346a1be7dee?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: "Women's Blouse Silk",
      description: 'Office elegant',
      price: 79,
      costPrice: 35,
      sku: 'APP-081',
      currentStock: 90,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1584273143981-43c26a0823b7?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Cardigan Sweater',
      description: 'Button up knit',
      price: 55,
      costPrice: 22,
      sku: 'APP-082',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1608234807905-4466023792f5?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Turtleneck Sweater',
      description: 'Wool blend',
      price: 65,
      costPrice: 28,
      sku: 'APP-083',
      currentStock: 80,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1611923134239-b9be5816e2ec?w=500&q=80',
      category: 'Apparel',
      sizes: ['S', 'M', 'L', 'XL'],
    },
    {
      wholesalerId: w2,
      name: 'Beanie Hat',
      description: 'Ribbed knit',
      price: 19,
      costPrice: 6,
      sku: 'APP-084',
      currentStock: 300,
      minStock: 50,
      imageUrl: 'https://images.unsplash.com/photo-1576822441727-42f6b0f0c4ee?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Baseball Cap',
      description: 'Adjustable back',
      price: 25,
      costPrice: 8,
      sku: 'APP-085',
      currentStock: 250,
      minStock: 40,
      imageUrl: 'https://images.unsplash.com/photo-1588850561407-ed78c282e89b?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Leather Belt Black',
      description: 'Classic buckle',
      price: 35,
      costPrice: 12,
      sku: 'APP-086',
      currentStock: 150,
      minStock: 30,
      imageUrl: 'https://images.unsplash.com/photo-1624222247344-550fb8ec5522?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Leather Belt Brown',
      description: 'Classic buckle',
      price: 35,
      costPrice: 12,
      sku: 'APP-087',
      currentStock: 150,
      minStock: 30,
      imageUrl: 'https://images.unsplash.com/photo-1614164185128-e4ec99c436d7?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Sneakers White',
      description: 'Casual everyday',
      price: 79,
      costPrice: 35,
      sku: 'APP-088',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1549298916-b41d501d3772?w=500&q=80',
      category: 'Apparel',
      sizes: ['7', '8', '9', '10', '11'],
    },
    {
      wholesalerId: w2,
      name: 'Running Shoes Black',
      description: 'Performance mesh',
      price: 120,
      costPrice: 50,
      sku: 'APP-089',
      currentStock: 80,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=500&q=80',
      category: 'Apparel',
      sizes: ['7', '8', '9', '10', '11'],
    },
    {
      wholesalerId: w2,
      name: 'Leather Boots',
      description: 'Waterproof',
      price: 150,
      costPrice: 70,
      sku: 'APP-090',
      currentStock: 50,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1520639889313-7272a74b1c73?w=500&q=80',
      category: 'Apparel',
      sizes: ['7', '8', '9', '10', '11'],
    },
    {
      wholesalerId: w2,
      name: 'Dress Shoes Oxford',
      description: 'Formal leather',
      price: 130,
      costPrice: 60,
      sku: 'APP-091',
      currentStock: 60,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1449241517440-ddef03a76ac5?w=500&q=80',
      category: 'Apparel',
      sizes: ['7', '8', '9', '10', '11'],
    },
    {
      wholesalerId: w2,
      name: 'Socks 5-Pack White',
      description: 'Cotton crew',
      price: 15,
      costPrice: 5,
      sku: 'APP-092',
      currentStock: 400,
      minStock: 80,
      imageUrl: 'https://images.unsplash.com/photo-1582966239103-57501004183f?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Socks 5-Pack Black',
      description: 'Cotton crew',
      price: 15,
      costPrice: 5,
      sku: 'APP-093',
      currentStock: 400,
      minStock: 80,
      imageUrl: 'https://images.unsplash.com/photo-1582966239103-57501004183f?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Underwear 3-Pack',
      description: 'Boxer briefs',
      price: 25,
      costPrice: 10,
      sku: 'APP-094',
      currentStock: 300,
      minStock: 60,
      imageUrl: 'https://images.unsplash.com/photo-1590243058048-526b3d853320?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Scarf Plaid',
      description: 'Winter warm',
      price: 29,
      costPrice: 10,
      sku: 'APP-095',
      currentStock: 100,
      minStock: 20,
      imageUrl: 'https://images.unsplash.com/photo-1520903920243-00d872a2d1c9?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Gloves Leather',
      description: 'Touchscreen compatible',
      price: 45,
      costPrice: 18,
      sku: 'APP-096',
      currentStock: 80,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1542152862-bc409392e20b?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Sunglasses Aviator',
      description: 'Polarized lenses',
      price: 89,
      costPrice: 30,
      sku: 'APP-097',
      currentStock: 120,
      minStock: 25,
      imageUrl: 'https://images.unsplash.com/photo-1473496169904-658ba7c44d8a?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Sunglasses Wayfarer',
      description: 'Classic frame',
      price: 79,
      costPrice: 25,
      sku: 'APP-098',
      currentStock: 150,
      minStock: 30,
      imageUrl: 'https://images.unsplash.com/photo-1511499767010-a588a5994441?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Backpack Canvas',
      description: 'Laptop sleeve inside',
      price: 59,
      costPrice: 20,
      sku: 'APP-099',
      currentStock: 90,
      minStock: 15,
      imageUrl: 'https://images.unsplash.com/photo-1553062407-98eeb64c6a62?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
    {
      wholesalerId: w2,
      name: 'Duffel Bag',
      description: 'Gym and travel',
      price: 69,
      costPrice: 25,
      sku: 'APP-100',
      currentStock: 70,
      minStock: 10,
      imageUrl: 'https://images.unsplash.com/photo-1544644181-1484b3fdfc62?w=500&q=80',
      category: 'Apparel',
      sizes: [],
    },
  ];

  await prisma.product.createMany({ data: hardcodedProducts });

  // 2. Generate 180 realistic products per category for the 11 categories (11 * 180 = 1980 products)
  console.log('  - Programmatically generating 1,980 products (180 per category)...');

  const categoryConfigs = {
    Electronics: {
      wholesalerId: w1,
      prefix: 'ELEC',
      brands: [
        'Sony',
        'Logitech',
        'Bose',
        'Samsung',
        'Anker',
        'Xiaomi',
        'Dell',
        'HP',
        'Sennheiser',
        'JBL',
      ],
      nouns: [
        'Headphones',
        'Speaker',
        'Keyboard',
        'Mouse',
        'Monitor',
        'Charger',
        'Power Bank',
        'Router',
        'Webcam',
        'Earbuds',
        'Hub',
        'Cable',
        'Stand',
        'Smart Plug',
        'Smart Bulb',
      ],
      priceMin: 15,
      priceMax: 999,
    },
    Apparel: {
      wholesalerId: w2,
      prefix: 'APP',
      brands: [
        'Nike',
        'Adidas',
        'Under Armour',
        "Levi's",
        'Zara',
        'H&M',
        'Patagonia',
        'Puma',
        'Champion',
        'Timberland',
      ],
      nouns: [
        'T-Shirt',
        'Jeans',
        'Hoodie',
        'Jacket',
        'Socks',
        'Shoes',
        'Hat',
        'Sweater',
        'Pants',
        'Sneakers',
        'Shorts',
        'Belt',
        'Scarf',
        'Gloves',
      ],
      priceMin: 12,
      priceMax: 120,
    },
    Grocery: {
      wholesalerId: wGrocery,
      prefix: 'GRO',
      brands: [
        'Organic Valley',
        'Heinz',
        'Quaker',
        'Dole',
        'Chobani',
        'Kirkland',
        "Kellogg's",
        'Nestle',
        'Kraft',
        'McCormick',
      ],
      nouns: [
        'Whole Milk',
        'Whole Wheat Bread',
        'Greek Yogurt',
        'Olive Oil',
        'Almonds',
        'Wildflower Honey',
        'Coffee Beans',
        'Green Tea',
        'Dark Chocolate',
        'Granola',
        'Chia Seeds',
        'Oat Milk',
        'Avocado',
        'Banana',
        'Brown Rice',
      ],
      priceMin: 2,
      priceMax: 25,
    },
    Sports: {
      wholesalerId: wSports,
      prefix: 'SPO',
      brands: [
        'Wilson',
        'Spalding',
        'Everlast',
        'Gatorade',
        'Nike Sports',
        'Decathlon',
        'Coleman',
        'CamelBak',
        'Rawlings',
        'Callaway',
      ],
      nouns: [
        'Yoga Mat',
        'Dumbbells',
        'Water Bottle',
        'Jump Rope',
        'Backpack',
        'Knee Sleeve',
        'Goggles',
        'Towel',
        'Gloves',
        'Tennis Racket',
        'Soccer Ball',
        'Trekking Poles',
        'Ankle Weights',
      ],
      priceMin: 10,
      priceMax: 150,
    },
    Books: {
      wholesalerId: wBooks,
      prefix: 'BOK',
      brands: [
        'Penguin',
        'HarperCollins',
        "O'Reilly",
        'Packt',
        'Macmillan',
        'Scholastic',
        'Pearson',
        'Simon & Schuster',
      ],
      nouns: [
        'Novel',
        'Biography',
        'Guidebook',
        'History Book',
        'Textbook',
        'Thriller Novel',
        'Mystery Novel',
        'Poetry Anthology',
        'Cookbook',
        'Sci-Fi Novel',
        'Self-Help Book',
      ],
      priceMin: 10,
      priceMax: 40,
    },
    Beauty: {
      wholesalerId: wBeauty,
      prefix: 'BEA',
      brands: [
        'CeraVe',
        'The Ordinary',
        "L'Oreal",
        'Neutrogena',
        'Sephora',
        'Clinique',
        'Olay',
        'Nivea',
        'Dove',
        'Garnier',
      ],
      nouns: [
        'Cleanser',
        'Moisturizer',
        'Serum',
        'Sunscreen',
        'Lip Balm',
        'Mascara',
        'Face Scrub',
        'Shampoo',
        'Conditioner',
        'Body Wash',
        'Essential Oil',
        'Face Mask',
      ],
      priceMin: 5,
      priceMax: 35,
    },
    'Home & Kitchen': {
      wholesalerId: wHome,
      prefix: 'HOM',
      brands: [
        'KitchenAid',
        'Ninja',
        'Instant Pot',
        'Cuisinart',
        'T-fal',
        'Pyrex',
        'Philips',
        'Dyson',
        'Shark',
        'Keurig',
      ],
      nouns: [
        'Blender',
        'Coffee Maker',
        'Air Fryer',
        'Knife Set',
        'Frying Pan',
        'Food Container',
        'Toaster',
        'Slow Cooker',
        'Kettle',
        'Hand Mixer',
        'Cutting Board',
        'Trash Can',
      ],
      priceMin: 15,
      priceMax: 250,
    },
    'Toys & Games': {
      wholesalerId: wToys,
      prefix: 'TOY',
      brands: [
        'Lego',
        'Hasbro',
        'Mattel',
        'Nerf',
        'Fisher-Price',
        'Ravensburger',
        'Bandai',
        'Hot Wheels',
        'Play-Doh',
      ],
      nouns: [
        'Board Game',
        'Puzzle',
        'Action Figure',
        'Building Blocks',
        'Card Game',
        'Remote Control Car',
        'Plush Toy',
        'Doll',
        'Train Set',
        'Water Gun',
      ],
      priceMin: 10,
      priceMax: 75,
    },
    Automotive: {
      wholesalerId: wAuto,
      prefix: 'AUT',
      brands: [
        "Meguiar's",
        'Mobil 1',
        'Rain-X',
        'Castrol',
        'Michelin',
        'Bosch',
        'Armor All',
        'Anker Auto',
        'Carhartt',
      ],
      nouns: [
        'Car Wax',
        'Phone Mount',
        'Seat Cover',
        'Tire Inflator',
        'Wiper Blade',
        'Engine Oil',
        'Floor Mats',
        'Jumper Cables',
        'Car Charger',
        'Microfiber Cloths',
        'Tire Pressure Gauge',
      ],
      priceMin: 12,
      priceMax: 120,
    },
    'Office Supplies': {
      wholesalerId: wOffice,
      prefix: 'OFF',
      brands: [
        'Moleskine',
        'Pilot',
        'BIC',
        'Sharpie',
        'Post-it',
        'Mead',
        'Paper Mate',
        'Fellowes',
        '3M',
        'AmazonBasics',
      ],
      nouns: [
        'Notebook',
        'Gel Pens',
        'Desk Organizer',
        'Stapler',
        'Paper Shredder',
        'Dry Erase Board',
        'Sticky Notes',
        'Paper Clips',
        'Scissors',
        'Desk Lamp',
        'File Folders',
      ],
      priceMin: 3,
      priceMax: 30,
    },
    'Pet Supplies': {
      wholesalerId: wPets,
      prefix: 'PET',
      brands: [
        'Purina',
        'Blue Buffalo',
        'Greenies',
        'KONG',
        'Frontline',
        'Furminator',
        'Catit',
        'Royal Canin',
        'Fancy Feast',
      ],
      nouns: [
        'Dog Food',
        'Cat Litter',
        'Pet Toy',
        'Leash & Collar',
        'Pet Bed',
        'Grooming Brush',
        'Water Fountain',
        'Cat Tree',
        'Pet Carrier',
        'Training Treats',
      ],
      priceMin: 8,
      priceMax: 60,
    },
  };

  const adjectives = [
    'Ergonomic',
    'Premium',
    'Eco-Friendly',
    'Smart',
    'Heavy-Duty',
    'Compact',
    'Pro',
    'Classic',
    'Portable',
    'Waterproof',
    'Ultra',
    'Wireless',
    'Deluxe',
    'Advanced',
    'Natural',
  ];
  const variants = [
    'Pro',
    'Max',
    'Ultra',
    'Lite',
    'Classic',
    'Sport',
    'Mini',
    'Plus',
    'Eco',
    'Elite',
  ];
  const colors = ['Black', 'White', 'Blue', 'Red', 'Grey', 'Silver', 'Green', 'Yellow', 'Pink'];

  const fakerProducts = [];

  for (const [category, config] of Object.entries(categoryConfigs)) {
    const { wholesalerId, prefix, brands, nouns, priceMin, priceMax } = config;
    const generatedNames = new Set();
    let skuIndex = 101; // Sku counter for this category

    for (let i = 0; i < 180; i++) {
      let name = '';
      let attempts = 0;

      // Keep generating name combinations until we get a unique one
      while (attempts < 50) {
        const brand = brands[Math.floor(Math.random() * brands.length)];
        const adjective = adjectives[Math.floor(Math.random() * adjectives.length)];
        const noun = nouns[Math.floor(Math.random() * nouns.length)];
        const variant = variants[Math.floor(Math.random() * variants.length)];
        const color = colors[Math.floor(Math.random() * colors.length)];

        name = `${brand} ${adjective} ${noun} (${variant}, ${color})`;
        if (!generatedNames.has(name)) {
          generatedNames.add(name);
          break;
        }
        attempts++;
      }

      // Fallback in case of extreme collision
      if (attempts >= 50) {
        name = `${brands[0]} Gen-${i} Product (${colors[0]})`;
      }

      const finalPrice = faker.number.float({ min: priceMin, max: priceMax, fractionDigits: 2 });
      const finalCostPrice = Math.round(finalPrice * (0.55 + Math.random() * 0.1) * 100) / 100;

      let sizes;
      if (category === 'Apparel') {
        sizes = ['S', 'M', 'L', 'XL'];
      } else if (category === 'Sports') {
        sizes = ['Standard', 'XL'];
      } else {
        sizes = ['Standard'];
      }

      const sku = `${prefix}-${String(skuIndex).padStart(4, '0')}`;
      skuIndex += 1;

      const currentStock = faker.number.int({ min: 30, max: 200 });
      const minStock = faker.number.int({ min: 5, max: 20 });
      const imageUrl = getRandomImage(category);

      fakerProducts.push({
        wholesalerId,
        name,
        description: `Highly reliable ${faker.commerce.productAdjective().toLowerCase()} product. Built with durable ${faker.commerce.productMaterial().toLowerCase()} materials.`,
        price: finalPrice,
        costPrice: finalCostPrice,
        sku,
        imageUrl,
        category,
        sizes,
        currentStock,
        minStock,
      });
    }
  }

  await prisma.product.createMany({ data: fakerProducts });

  // Load all inserted products to return their full objects (with IDs) for reviews seeding
  const allProducts = await prisma.product.findMany();

  // Create initial stocking inventory logs for all products to reflect manual stock initialization
  console.log('  - Creating initial stocking inventory logs...');
  const initialStockLogs = allProducts.map((p) => ({
    wholesalerId: p.wholesalerId,
    productId: p.id,
    changeAmount: p.currentStock,
    reason: 'MANUAL_ADJUSTMENT',
  }));

  // Batch insert inventory logs to avoid heap limits
  const batchSize = 500;
  for (let j = 0; j < initialStockLogs.length; j += batchSize) {
    const batch = initialStockLogs.slice(j, j + batchSize);
    await prisma.inventoryLog.createMany({ data: batch });
  }

  console.log(`✅ Seeding complete: loaded ${allProducts.length} total products in database.`);

  return allProducts;
}
