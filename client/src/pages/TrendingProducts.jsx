...
import { ArrowLeft, TrendingUp, Star, Store, Heart } from 'lucide-react';
...
export default function TrendingProducts() {
  ...
  const products = useMemo(() => {
    const all = infiniteData?.pages?.flatMap((page) => page.products) || [];
    // Cap at 100 trending products
    return all.slice(0, 100);
  }, [infiniteData]);

  const totalCount = Math.min(infiniteData?.pages?.[0]?.totalCount || 0, 100);
  const canLoadMore = hasNextPage && products.length < 100;
      ...
      <div className="mx-auto w-full max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight text-[#1e293b]">Trending Now</h1>
              <p className="text-sm text-[#64748b]">Top 100 trending products</p>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="mt-16 text-center">
            <p className="text-xl font-bold text-[#1e293b]">No trending products found</p>
            <p className="mt-2 text-sm text-[#64748b]">Check back later for trending items.</p>
          </div>
        ) : (
          <>
            ...
            {!canLoadMore && products.length > 12 && (
              <div className="mt-10 flex flex-col items-center gap-3">
                <p className="text-xs font-semibold text-[#64748b]">
                  You&apos;ve seen all {products.length} trending products
                </p>
              </div>
            )}
          </>
