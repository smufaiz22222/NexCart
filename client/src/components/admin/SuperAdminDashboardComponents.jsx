...
export function SuperAdminHeaderHero({ cards }) {
  return (
      ...
      <div className="grid gap-8 px-6 py-8 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-10">
        <div>
          ...
          <p className="mt-4 max-w-xl text-base leading-7 text-[#5d5247]">
            ...
            platform totals, compare seller performance, and drill into a wholesaler without leaving
            the page.
          </p>
        </div>
...
export function SuperAdminPendingApplications({
  pendingApplications = EMPTY_ARRAY,
...
export function SuperAdminBillingCommand({ totals }) {
  return (
    <Panel title="Billing Command" eyebrow="Dedicated subscription page" icon={BadgeIndianRupee}>
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <MetricCard
          label="Paid active"
          value={totals?.activePaidSubscriptions || 0}
          icon={CreditCard}
        />
        <MetricCard label="Active trials" value={totals?.activeTrials || 0} icon={Sparkles} />
        <MetricCard
      ...
      <div className="mt-5 rounded-[24px] bg-[#fcf7f0] p-5">
        <p className="text-sm leading-7 text-[#6b6155]">
          Subscription activation, duration pricing, trial visibility, and billing history now live
          on a separate workspace so this dashboard stays cleaner.
        </p>
        <Link
...
export function SuperAdminB2BApplications({ b2bApps = EMPTY_ARRAY, handleB2BAction }) {
  const pendingApps = b2bApps.filter((app) => app.verification === 'APPLIED');

  return (
    <section className="grid gap-6 xl:grid-cols-1">
      <Panel
        title="B2B Business Applications"
        eyebrow="Retail buyer business verification requests"
        icon={Shield}
      >
        <div className="space-y-3">
...
export function SuperAdminCharts({ monthlyRevenue, orderStatus, formatCurrency }) {
  return (
    <section className="grid gap-6 xl:grid-cols-[1.25fr_0.95fr]">
      ...
      <Panel title="Order Status Mix" eyebrow="Platform-wide" icon={ShoppingBag}>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  paddingAngle={3}
                >
                  {(orderStatus || EMPTY_ARRAY).map((entry, index) => (
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {(orderStatus || EMPTY_ARRAY).map((item, index) => (
            <div
              key={item.status}
...
export function SuperAdminWholesalerLeaders({
  topWholesalers = EMPTY_ARRAY,
  wholesalers = EMPTY_ARRAY,
...
export function SuperAdminTenantDetail({ isLoadingTenant, tenant, formatCurrency, formatDate }) {
  return (
