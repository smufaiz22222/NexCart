...
import { MetricCard, SectionCard } from './LayoutComponents';

export function LedgerHeaderSection({
  ocrInputRef,
  handleOcrFileChange,
  isOcrLoading,
...
export function LedgerMetricsSection({ hub, isLoading, formatCurrency }) {
  return (
    <>
...
export function LedgerOverviewTab({ hub, formatCurrency, setSelectedAccountId }) {
              ...
              <tbody className="divide-y divide-zinc-800">
                {hub.accounts.map((account) => (
                  <tr
                    ...
                    tabIndex={0}
                    role="button"
                    className="bg-[#121212] hover:bg-zinc-800/30 cursor-pointer transition-colors group focus:outline-none focus:ring-1 focus:ring-amber-500/50"
              ...
              <div key={party.id} className="rounded-2xl border border-zinc-800 bg-[#0d0d0d] p-4">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-xl border border-zinc-800 bg-[#131313] p-3">
                    <p className="text-xs uppercase tracking-[0.18em] text-zinc-500">Receivable</p>
...
export function LedgerSalesTab({
  hub,
  formatCurrency,
  setShowSaleModal,
  verifyBankPaymentMutation,
  refetch,
}) {
  return (
    <SectionCard
          ...
          <tbody className="divide-y divide-zinc-800">
            {hub.sales.map((sale) => (
              <tr key={sale.id} className="bg-[#121212] hover:bg-zinc-900/30 transition">
                <td className="px-4 py-3 text-zinc-400">{new Date(sale.date).toLocaleString()}</td>
...
export function LedgerPurchasesTab({
  hub,
  formatCurrency,
  setOcrPurchaseValues,
  setShowPurchaseModal,
        ...
        <div className="overflow-x-auto rounded-2xl border border-zinc-800">
          <table className="min-w-full divide-y divide-zinc-800 text-sm">
            ...
            <tbody className="divide-y divide-zinc-800">
              ...
              {(hub.paymentInstruments || []).filter((inst) => inst.type === 'PAYABLE').length ===
              0 ? (
                <tr>
                  <td colSpan="9" className="px-4 py-6 text-center text-zinc-500">
...
export function LedgerPartiesTab({
  hub,
  partyCategory,
  setPartyCategory,
  setShowPartyModal,
                ...
                <tr key={party.id} className="bg-[#121212] hover:bg-zinc-900/30 transition">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      <button
                        type="button"
                        onClick={() => setSettlementContext({ partyId: party.id, direction: 'IN' })}
                        disabled={!canReceive}
                        className={cn(
...
export function LedgerHistoryTab({ hub, ledgerFilter, setLedgerFilter, formatCurrency }) {
