...
export default function RfqCard({
  ...
  return (
    ...
    >
            ...
            {rfq.counterPrice ? (
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  >
                    ₹
                    {(parseFloat(rfq.counterPrice) * (rfq.counterQuantity ?? rfq.quantity)).toFixed(
                      2
                    )}
                  </p>
                </div>
      ...
      {isWholesaler && rfq.status === 'PENDING' && !hasEnoughStockForSeller && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-extrabold uppercase tracking-wide">Insufficient Inventory Stock</p>
              <p className="mt-0.5 opacity-90">
                Current Stock: <span className="font-bold font-mono">{currentStock}</span> units.
                Requested: <span className="font-bold font-mono">{rfq.quantity}</span> units.
              </p>
            </div>
          </div>
      ...
      {/* Counter offer sub-form (Wholesaler only) */}
      {isCounterOpen && (
        <SellerCounterForm
          rfq={rfq}
      ...
      {/* Counter back sub-form (Buyer/Customer only) */}
      {isBuyerCounterOpen && (
        <BuyerCounterForm
          rfq={rfq}
