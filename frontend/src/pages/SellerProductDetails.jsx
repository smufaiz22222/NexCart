...
export default function SellerProductDetails() {
  ...
  const stockStatus = useMemo(() => {
    if (!product) return { label: 'Unknown', className: 'bg-zinc-800/60 text-zinc-300 border-zinc-700' };
    if (product.currentStock === 0) return { label: 'Out of Stock', className: 'bg-red-500/10 text-red-300 border-red-500/20' };
    if (product.currentStock <= (product.minStock || 10)) return { label: 'Low Stock', className: 'bg-amber-500/10 text-amber-300 border-amber-500/20' };
  ...
  if (!product) {
    return (
      <div className="space-y-4">
        <Link to="/wholesaler/products" className="inline-flex items-center text-sm font-semibold text-amber-400 hover:text-amber-300">
    ...
    <div className="space-y-6 text-white">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-400">{product.description || 'No description available for this product yet.'}</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="grid gap-6 md:grid-cols-[220px_1fr]">
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap gap-3">
                <span className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-bold uppercase tracking-[0.25em] ${stockStatus.className}`}>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <DetailCard label="Last Updated" value={new Date(product.updatedAt).toLocaleString()} />
              </div>
            </div>
          </div>
        </section>
        ...
        <aside className="space-y-6">
          <SummaryPanel
            ...
            description={margin >= 0 ? 'Current gross margin per unit before expenses.' : 'Cost price is higher than selling price.'}
          />
          <SummaryPanel
            icon={Boxes}
          ...
          <div className="rounded-[28px] border border-zinc-800 bg-[#141414] p-5 shadow-[0_14px_35px_rgba(0,0,0,0.28)]">
            <div className="flex items-start">
              ...
              <div className="ml-3">
                <p className="text-xs font-bold uppercase tracking-[0.3em] text-zinc-500">Product Identity</p>
                <div className="mt-3 space-y-2 text-sm text-zinc-300">
                  <p><span className="text-zinc-500">Category:</span> {product.category || 'General'}</p>
                  <p><span className="text-zinc-500">SKU:</span> {product.sku || 'Not set'}</p>
                  <p><span className="text-zinc-500">Created:</span> {new Date(product.createdAt).toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleSave} className="max-h-[calc(90vh-88px)] overflow-y-auto p-6">
              <div className="grid gap-5">
                <FormField label="Product Name *">
                  <input required type="text" name="name" value={formData.name} onChange={handleChange} className={inputClassName} />
                </FormField>

                <FormField label="Description">
                  <textarea name="description" value={formData.description} onChange={handleChange} rows="4" className={`${inputClassName} resize-none`} />
                </FormField>

                <div className="grid gap-5 sm:grid-cols-2">
                  <FormField label="Category">
                    <input type="text" name="category" value={formData.category} onChange={handleChange} className={inputClassName} />
                  </FormField>
                  <FormField label="SKU *">
                    <input required type="text" name="sku" value={formData.sku} onChange={handleChange} className={`${inputClassName} font-mono`} />
                  </FormField>
                </div>

                <div className="grid gap-5 sm:grid-cols-2">
                  <FormField label="Selling Price (₹) *">
                    <input required type="number" min="0" step="0.01" name="price" value={formData.price} onChange={handleChange} className={inputClassName} />
                  </FormField>
                  <FormField label="Cost Price (₹)">
                    <input type="number" min="0" step="0.01" name="costPrice" value={formData.costPrice} onChange={handleChange} className={inputClassName} />
                  </FormField>
                </div>

                <div className="grid gap-5 sm:grid-cols-2">
                  <FormField label="Current Stock">
                    <input type="number" min="0" name="currentStock" value={formData.currentStock} onChange={handleChange} className={inputClassName} />
                  </FormField>
                  <FormField label="Minimum Stock Alert">
                    <input type="number" min="0" name="minStock" value={formData.minStock} onChange={handleChange} className={inputClassName} />
                  </FormField>
                </div>

                <FormField label="Image URL">
                  <input type="url" name="imageUrl" value={formData.imageUrl} onChange={handleChange} className={inputClassName} />
                </FormField>
              </div>
...
function DetailCard({ label, value, mono = false }) {
  return (
    <div className="rounded-2xl border border-zinc-800 bg-black/20 p-4">
      ...
      <p className={`mt-3 text-sm font-semibold text-white ${mono ? 'break-all font-mono text-xs' : ''}`}>{value}</p>
    </div>
  );
}

function SummaryPanel({ icon: Icon, title, value, description }) {
  return (
    <div className="rounded-[28px] border border-zinc-800 bg-[#141414] p-5 shadow-[0_14px_35px_rgba(0,0,0,0.28)]">
...
function FormField({ label, children }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-bold uppercase tracking-[0.24em] text-zinc-500">{label}</span>
      {children}
    </label>
  );
}
