    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center text-zinc-500 space-y-4">
        ...
        <button
          ...
          className="mt-4 text-amber-500 hover:text-amber-400 font-medium hover:underline"
        >
        ...
        <div className="bg-[#1c1c1c] rounded-xl shadow-2xl border border-zinc-800 p-6 md:p-8 flex flex-col md:flex-row items-start gap-8 md:gap-12">
          ...
          <div className="w-full md:w-2/3 flex flex-col justify-start">
            ...
            <p className="text-sm text-zinc-400 flex items-center mb-6 border-b border-zinc-800 pb-6">
              ...
              Sold by:{' '}
              <span className="font-bold text-zinc-200 ml-1.5">
          ...
          {isLoadingSimilar ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((item) => (
                <div
                  key={item}
                  className="bg-[#0a0a0a] border border-zinc-800 rounded-lg p-4 animate-pulse"
                >
        ...
        <div className="mt-8 bg-[#1c1c1c] rounded-xl shadow-2xl border border-zinc-800 p-8 md:p-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div className="md:col-span-1 bg-[#0a0a0a] rounded-lg p-6 border border-zinc-800 h-fit shadow-inner">
              <h3 className="text-sm font-bold text-amber-500 uppercase tracking-widest mb-5">
                Write a Review
              </h3>
                ...
                <div className="mb-5">
                  ...
                  <div className="flex space-x-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star
                          className={`h-7 w-7 transition-colors ${rating >= star ? 'fill-amber-500 text-amber-500 drop-shadow-[0_0_5px_rgba(245,158,11,0.5)]' : 'text-zinc-700'}`}
                        />
                      </button>
                    ))}
                  </div>
            ...
            <div className="md:col-span-2 space-y-6">
              {product.reviews && product.reviews.length > 0 ? (
                product.reviews.map((review, idx) => (
                  <div
                    key={idx}
                    className="border-b border-zinc-800 pb-6 last:border-0 hover:bg-zinc-800/20 p-4 -mx-4 rounded-lg transition-colors"
                  >
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="flex mr-3">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`h-4 w-4 ${i < review.rating ? 'fill-amber-500 text-amber-500' : 'text-zinc-700'}`}
                            />
                          ))}
                        </div>
                      </div>
