    ...
    <div className="space-y-6 font-sans selection:bg-amber-500/30 selection:text-amber-200">
      ...
      {isLoading ? (
        ...
        <div className="bg-[#1c1c1c] rounded-lg shadow-xl border border-dashed border-zinc-700 p-12 flex flex-col items-center justify-center text-center">
          ...
          <p className="mt-2 text-zinc-400 max-w-sm">When you add stock or process an order, the immutable logs will appear here.</p>
        </div>
      ) : (
            ...
            <table className="min-w-full divide-y divide-zinc-800">
              <thead className="bg-[#0a0a0a]">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-bold text-amber-500/80 uppercase tracking-widest">Reason</th>
                  <th className="px-6 py-4 text-right text-xs font-bold text-amber-500/80 uppercase tracking-widest">Change</th>
                </tr>
              </thead>
                ...
                {logs.map((log) => (
                  <tr key={log.id} className="hover:bg-zinc-800/30 transition-colors group">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-bold text-white group-hover:text-amber-400 transition-colors">{log.product.name}</div>
                      <div className="text-[11px] text-zinc-500 font-mono tracking-wider mt-0.5">SKU: {log.product.sku}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1c1c1c] rounded-lg shadow-2xl w-full max-w-md overflow-hidden border border-zinc-800 flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5">Select Product *</label>
                <select 
                  required 
                  name="productId" 
                ...
                >
                  <option value="" disabled className="text-zinc-600">-- Choose a product --</option>
                  {products.map(p => (
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5">Quantity Change (+ or -) *</label>
                <input 
                  required 
                  type="number" 
                  name="changeAmount" 
                ...
                />
                <p className="text-[11px] font-medium text-zinc-500 mt-1.5 uppercase tracking-wide">Use a negative number to remove stock.</p>
              </div>
